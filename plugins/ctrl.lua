...
local function disable_channel(receiver, to_id)
	...
	save_config()
	return 'کس کشا ممدو ناراحت کردین رفت :|'
end
...
local function run(msg, matches)
	if permissions(msg.from.id, msg.to.id, "mamad") then
		local receiver = get_receiver(msg)
		-- Enable a channel
 ...
 if string.match then
   ...
   local text = text..'آپدیت شد'
	...
	if matches[1] == 'rl' and is_sudo(msg) then
		...
		reload_plugins(true)
		post_msg(receiver, "ممد رفت حموم :|", ok_cb, false)
		return "ممد رفت حموم :|"
	end
end
